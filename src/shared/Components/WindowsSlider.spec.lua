...
return function(t)
	...
	t.test("mounts the slider area with gutter, knob, and end labels", function()
		withSlider({
			...
			-- Input runs through a capturing drag detector; nothing between
			-- it and the press may be Active (it would sink the press)
			t.expect(frame.SliderArea.DragDetector.ClassName).toBe("UIDragDetector")
			t.expect(frame.SliderArea.DragDetector.DragStyle).toBe(Enum.UIDragDetectorDragStyle.Scriptable)
			t.expect(frame.SliderArea.Gutter.ClassName).toBe("ImageLabel")
			t.expect(frame.SliderArea.Slider.ClassName).toBe("ImageLabel")
			t.expect(frame.SliderArea.Slider.Active).toBeFalsy()
