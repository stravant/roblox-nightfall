...
return function(t)
	...
	t.test("shows the four tabs with the Settings tab selected", function()
		withView(function(view, gui)
			local tabPanel = gui.Menu.TabPanel
			t.expect(tabPanel.TabStrip.Settings.Text.Text).toBe("Settings")
			...
			t.expect(tabPanel.TabStrip.Settings.ImageRectOffset).toBe(Vector2.new(22, 0))
			t.expect(tabPanel.TabStrip.Settings.ZIndex).toBe(2)
			t.expect(tabPanel.TabStrip.Skips.ImageRectOffset).toBe(Vector2.new(44, 0))
			t.expect(tabPanel.Tabs.Settings.Visible).toBeTruthy()
	...
	t.test("settings tab has both sliders centered by SoundManager (volume 1 -> value 0)", function()
		withView(function(view, gui)
			local soundTab = gui.Menu.TabPanel.Tabs.Settings
	...
	t.test("a battle context adds a default-selected Databattle tab", function()
		...
		local ok, err = pcall(function()
			...
			t.expect(tabs.Settings.Visible).toBeFalsy()
			...
			t.expect(gui.Menu.WindowTitle.Text).toBe("Game Menu")
			t.expect(gui.Menu.TabPanel.Tabs:FindFirstChild("Databattle")).toBe(nil)
			t.expect(gui.Menu.TabPanel.Tabs.Settings.Visible).toBeTruthy()
		end)
		if view then
			ReactRoblox.act(function()
				view:Destroy()
