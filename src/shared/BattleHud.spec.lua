...
return function(t)
	...
	t.test("mounts with the programs window and an empty command row", function()
		withHud(function(hud, gui)
			local inset = gui.ProgramsWindow.Inset
			t.expect(inset.hack.CountLabel.Text).toBe("2x")
			t.expect(inset.hack.NameLabel.Text).toBe(Scripts.hack.Name)
			t.expect(inset.slingshot.CountLabel.Text).toBe("0x")
			t.expect(gui:FindFirstChild("InfoWindow")).toBe(nil)
			-- Command row exists but holds only its layout
			t.expect(#gui.CommandRow:GetChildren()).toBe(1)
		end)
	end)
	...
	t.test("selecting a unit definition fills the info window and command row", function()
		withHud(function(hud, gui)
			...
			t.expect(gui.InfoWindow.WindowTitle.Text).toBe(Scripts.hack.Name)
			t.expect(gui.InfoWindow.Inset.FlavorText.Text).toBe(Scripts.hack.Desc)
			-- Dummy definition has no moves left: no move button
			t.expect(gui.CommandRow:FindFirstChild("move")).toBe(nil)
			for _, command in pairs(Scripts.hack.CommandList) do
				t.expect(gui.CommandRow:FindFirstChild(command.Id) ~= nil).toBeTruthy()
			end
		end)
	end)
	...
	t.test("a movable unit gets a move button and command selection highlights", function()
		withHud(function(hud, gui)
			...
			t.expect(gui.CommandRow:FindFirstChild("move") ~= nil).toBeTruthy()
			local commandId = Scripts.hack.CommandList[1].Id
			ReactRoblox.act(function()
				hud:SetSelectedCommand(commandId)
			end)
			t.expect(gui.CommandRow[commandId].ImageColor3).toBe(Color3.new(0, 0, 1))
			-- Move selection maps nil -> the move button
			ReactRoblox.act(function()
				hud:SetSelectedCommand(nil)
			end)
			t.expect(gui.CommandRow.move.ImageColor3).toBe(Color3.new(0, 0, 1))
		end)
	end)
	...
	t.test("UpdateCount rewrites the row and GetCount tracks it", function()
		withHud(function(hud, gui)
			...
			t.expect(gui.ProgramsWindow.Inset.hack.CountLabel.Text).toBe("1x")
	...
	t.test("program window visibility and Hide", function()
		withHud(function(hud, gui)
			...
			t.expect(gui:FindFirstChild("ProgramsWindow")).toBe(nil)
			ReactRoblox.act(function()
				hud:Hide()
			end)
			t.expect(gui:FindFirstChild("CommandRow")).toBe(nil)
		end)
	end)
end
