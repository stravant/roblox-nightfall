...
function Tutorial:PlayTutorial(container, netmapView, mainDialogue, onBattleWon)
	...
	-- Upload phase
	tutorialDialogue:SetText("This is a databattle: your scripts against the node's defenses. Click this upload spot to deploy your first script.")
	waitForClick(4, 5) -- hack
	tutorialDialogue:SetText("Click the Hack script to upload it.")
	gameView:ShowTutorialArrowProgramList('hack')
	gameState.UnitAdded:wait()
	tutorialDialogue:SetText("One more: click this upload spot.")
	waitForClick(3, 3)
	tutorialDialogue:SetText("Upload the Slingshot script.")
