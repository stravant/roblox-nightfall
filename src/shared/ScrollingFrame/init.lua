...
local MakeMaid = require(script.Maid).MakeMaid
...
function Scroller:GetScale(TimesOverBounds)
	return (1 - 0.5 ^ math.abs(TimesOverBounds)) * 0.5
end
...
function ScrollingFrame:GetVelocityTracker(Strength)
	...
	return function()
		...
		self.Scroller.Velocity = self.Scroller.Velocity - (Delta / (0.05 + Elapsed)) * Strength
	end
end
